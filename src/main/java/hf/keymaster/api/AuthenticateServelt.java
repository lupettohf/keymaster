...
@WebServlet(name = "AuthenticateServelt", displayName="AuthenticateServelt", urlPatterns = {"/api/v1/authenticate/*"})
								...
								if(ow.getLicenseID() == lic.getID())
								{
									if(OwnedLicenseDAO.isActive(ow))
									{
										ApiResponse apiResponse = new ApiResponse(
												...
												lic.getDescription(),
												lic.getType(),
												ow.getActivationEpoch()
												);
										ObjectMapper mapper = new ObjectMapper(); 
										String Json = mapper.writeValueAsString(apiResponse); 
										out.print(Json);
										out.flush();
									} else { /* License exsist but expired */ }
								}
							}
						}
					} else { /*No license */ }
