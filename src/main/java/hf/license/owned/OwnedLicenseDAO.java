...
public class OwnedLicenseDAO {
	...
	public static boolean activateLicense(User user, License license, Key key)
	{
		...
		if(_tf.isRedeemed()) { return false; }
	...
	public static OwnedLicense getOwnedLicense(int id)
	{
		...
		try {
			...
			if(rs.next())
			{
				return new OwnedLicense(
						rs.getInt("id"), 
						rs.getInt("ownerid"), 
						rs.getInt("licenseid"), 
						rs.getInt("keyid"), 
						rs.getLong("activationepoch")
				);
			} 
	...
	public static List<OwnedLicense> getOwnedLicenses(User user)
	{
		...
		try {
			...
			while(rs.next())
			{
				ownedLicense.add(new OwnedLicense(
						rs.getInt("id"), 
						rs.getInt("ownerid"), 
						rs.getInt("licenseid"), 
						rs.getInt("keyid"), 
		...
		if(ownedLicense.isEmpty()) { return null; }
		else { return ownedLicense; }
	}
	...
	public static boolean renewLicense(OwnedLicense owned, Key key)
	{
		...
		{
			if(_tf.isRedeemed()) { return false; }
			if(_tf.getLicenseID() != _ow.getLicenseID()) { return false; }
	...
	public static boolean isActive(OwnedLicense owned)
	{
		...
		if(_ow == null || _lic == null) { return false; }
